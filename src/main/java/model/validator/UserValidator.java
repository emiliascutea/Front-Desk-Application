...
public class UserValidator {
    ...
    public void validateLogin(String username, String password) {
        ...
        validatePasswordMatch(username, password);
    }

    public void validateReport(Long employeeId){
        errors.clear();
        validateUserExists(employeeId);
    }

    public void validateUpdate(String username, String password){
        errors.clear();
        validateEmail(username);
        validatePasswordLength(password);
        validatePasswordSpecial(password);
    ...
    private void validateUserExists(Long id){
        if(
                authenticationService.findById(id) == null){
            errors.add("Employee with id %d does not exist".formatted(id));
        }
    }
    ...
    private void validatePasswordMatch(String email, String password) {
        final Response<Boolean> response = userRepository.existsByUsername(email);
        if (response.hasErrors()) {
            errors.add(response.getFormattedErrors());
        } else {
            if (!authenticationService.encodePassword(password).equals(userRepository.getPassword(email))) {
